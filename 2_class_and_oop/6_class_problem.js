...
const iveMembersInstance = iveMembers.map((member)=> new FemaleIdol(member.name, member.year));

console.log(iveMembersInstance);

const btsMembersInstance = btsMembers.map((member)=> new MaleIdol(member.name, member.year));

console.log(btsMembersInstance);

const ive = new IdolGroup('아이브', iveMembersInstance);

console.log(ive);

const bts = new IdolGroup('BTS', btsMembersInstance);

console.log(bts);

const korea = new Country('대한민국', [ive, bts]);
...
const allTogether = new Country(
    ...
    [
        ...
        new IdolGroup(
            'BTS',
            iveMembers.map((member)=> new MaleIdol(member.name, member.year),),
        ),
    ],
);
